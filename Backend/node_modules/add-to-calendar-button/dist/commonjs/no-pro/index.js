  ...
  for (let i = loopStart; i <= loopEnd; i++) {
    ...
    const timeAddon = (function () {
      ...
      if (data.dates[`${i}`].timeZone && data.dates[`${i}`].timeZone !== '') {
        const timeZoneBlock = tzlibActions.tzlib_get_ical_block(data.dates[`${i}`].timeZone);
...
function atcb_generate_time(data, style = 'delimiters', targetCal = 'general', addTimeZoneOffset = false) {
  if (data.startTime && data.startTime !== '' && data.endTime && data.endTime !== '') {
    ...
    const offsetStart = tzlibActions.tzlib_get_offset(data.timeZone, data.startDate, data.startTime);
    const offsetEnd = tzlibActions.tzlib_get_offset(data.timeZone, data.endDate, data.endTime);
...
function atcb_translate_via_time_zone(date, time, baseTimeZone, targetTimeZone) {
  ...
  const offset = tzlibActions.tzlib_get_offset(baseTimeZone, date, time); 
...
module.exports = { atcb_action, i18nStrings, cssStyles: atcbCssTemplate };